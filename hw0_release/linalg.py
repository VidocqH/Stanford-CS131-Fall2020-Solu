...
def dot_product(a, b):
    ...
    out = None
    ### YOUR CODE HERE
    out = sum(np.transpose(a) * b).reshape(1, 1)
    ### END YOUR CODE
    return out
...
def get_eigen_values_and_vectors(M, k):
    ...
    eigenvectors = []
    ### YOUR CODE HERE
    eigVal, eigVec = eigen_decomp(M)
    ind = np.argsort(eigVal)[-k:][::-1]
    for i in range(k):
        eigenvalues.append(eigVal[i])
        eigenvectors.append(eigVec[i])
    ### END YOUR CODE
