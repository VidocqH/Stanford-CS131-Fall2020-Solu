...
def rotate2d(point, theta):
    ...
    ## YOUR CODE HERE
    x, y = point
    res_x = np.cos(theta) * x - np.sin(theta) * y
    res_y = np.cos(theta) * y + np.sin(theta) * x
...
def rotate_image(input_image, theta):
    ...
    ## YOUR CODE HERE
    rowsLen = int(input_rows / 2)
    colsLen = int(input_cols / 2)
    for i in range(-rowsLen, rowsLen):
        for j in range(-colsLen, colsLen):
            res_x, res_y = rotate2d(np.array([i, j]), theta)
            if int(res_x) + rowsLen in range(input_rows) and int(res_y) + colsLen in range(input_cols):
                output_image[i + rowsLen, j + colsLen, :] = input_image[int(res_x) + rowsLen, int(res_y) + colsLen, :]
    ### END YOUR CODE
